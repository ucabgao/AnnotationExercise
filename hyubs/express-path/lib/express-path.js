...
var expressPath = function (app, routeParam: string, opt) {
	...
	var options = {
		...
		middlewaresPath: ((typeof opt.middlewaresPath === 'undefined') ? ("middlewares" + path.sep) : opt.controllersPath),
	...
	var map = routeMap.eval(MODULE_PATH + routeParam);
	...
	if(routeMap.isValid(map)) {
		...
		for(var i in map) {
			...
			if(mapRow.length > 1) {
				httpRequestMethod = mapRow[mapRow.length - 1];
			}
			else {
			...
			var method = getModuleMethod(MODULE_PATH + options.controllersPath, cmPath);

			// Loop through all the middleware-method paths and retrieve each of the methods
			var middlewares = [];
			mmPaths.forEach(function(mmPath) {
				var middlewareMethod = getModuleMethod(MODULE_PATH + options.middlewaresPath, mmPath);
			...
			if(options.verbose) {
				...
				if(mmPaths.length > 0) {
					console.log('    - middlewares (' + mmPaths.length + ')');
					mmPaths.forEach(function(mmPath) {
						console.log('        - ' + mmPath);
					})
				}
